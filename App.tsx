    ...
    <div className={`w-full min-h-screen bg-casino-bg text-white font-body overflow-hidden flex flex-col ${selectedGame.bgImage}`}>
      ...
      <header className="fixed top-0 w-full z-[100] bg-gradient-to-b from-[#4c1d95] to-[#2e1065] px-2 md:px-4 flex justify-between items-center shadow-[0_8px_15px_rgba(0,0,0,0.6)] h-[35px] md:h-[64px]">
          <div className="flex items-center gap-2 md:gap-6 relative z-10 w-1/2">
             ...
             <div className="flex items-center gap-1 drop-shadow-md shrink-0 bg-black/20 rounded-full px-2 py-0.5 md:py-1 shadow-inner">
                 <span className="text-sm md:text-xl filter drop-shadow">💎</span>
          ...
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
             <button onClick={() => openShop('COINS')} className="bg-gradient-to-b from-green-500 to-green-700 px-4 md:px-8 py-1 md:py-2 rounded-full shadow-[0_4px_0_rgb(21,128,61)] hover:shadow-[0_2px_0_rgb(21,128,61)] hover:translate-y-[2px] transition-all active:translate-y-[4px] active:shadow-none flex items-center gap-2">
                 ...
                 <div className="text-base md:text-2xl animate-pulse">🪙</div>
             </button>
          </div>
                 ...
                 <div className="flex items-center gap-2">
                    {player.level >= 5 && (
                        <button 
                            ...
                            className={`w-8 h-8 md:w-10 md:h-10 rounded-full bg-pink-600 border border-pink-400 flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 transition-all ${piggyGlow ? 'ring-4 ring-pink-300 shadow-[0_0_20px_#f472b6] scale-110' : ''}`}
                        >
              ...
              <div className="w-full max-w-7xl mx-auto flex flex-row items-center justify-between gap-2 md:gap-4 h-full">
                  <div className="flex items-center gap-1 md:gap-2 shrink-0">
                      <div className="bg-black/40 p-1 rounded-2xl flex items-center shadow-inner relative h-full ring-1 ring-white/10">
                          <button onClick={() => { setBetIndex(prev => Math.max(0, prev - 1)); audioService.playClick(); }} disabled={betIndex === 0 || status === GameStatus.SPINNING || status === GameStatus.STOPPING || autoMaxBet} className="w-8 h-8 md:w-12 md:h-12 rounded-xl bg-gradient-to-b from-gray-700 to-gray-900 hover:brightness-110 flex items-center justify-center text-lg md:text-2xl shadow-lg active:translate-y-0.5 disabled:opacity-50">-</button>
                          ...
                          <button onClick={() => { setBetIndex(prev => Math.min(availableBets.length - 1, prev + 1)); audioService.playClick(); }} disabled={betIndex === availableBets.length - 1 || status === GameStatus.SPINNING || status === GameStatus.STOPPING || autoMaxBet} className="w-8 h-8 md:w-12 md:h-12 rounded-xl bg-gradient-to-b from-gray-700 to-gray-900 hover:brightness-110 flex items-center justify-center text-lg md:text-2xl shadow-lg active:translate-y-0.5 disabled:opacity-50">+</button>
                      </div>
                      <div className="flex flex-col md:flex-row gap-1 items-center">
                          <button onClick={() => { if (!autoMaxBet) setBetIndex(availableBets.length - 1); audioService.playClick(); }} disabled={status === GameStatus.SPINNING || status === GameStatus.STOPPING || autoMaxBet} className={`w-10 h-8 md:w-14 md:h-12 rounded-xl bg-gradient-to-b from-red-600 to-red-800 hover:brightness-110 shadow-lg flex items-center justify-center active:translate-y-0.5 disabled:opacity-50 transition-all ${autoMaxBet ? 'grayscale opacity-50' : ''}`}>
                              <span className="text-[10px] md:text-xs font-black text-white uppercase leading-none transform -rotate-6">MAX</span>
                          </button>
                          <label className="flex items-center gap-1 cursor-pointer bg-black/30 px-1 md:px-2 py-1 rounded-lg ring-1 ring-white/5">
                              <input type="checkbox" checked={autoMaxBet} onChange={(e) => setAutoMaxBet(e.target.checked)} className="w-3 h-3 md:w-4 md:h-4 text-red-600 rounded focus:ring-0" />
                              <span className="text-[8px] md:text-[10px] font-bold uppercase text-gray-300 leading-none">Auto Max</span>
                          </label>
                      </div>
                  </div>
